...
fun sendGet(url: String, param: Map<String, String>? = null, header: Map<String, String>? = null): String {
	var result = ""
	var `in`: BufferedReader? = null
	try {
		val paramSB = StringBuilder()
		val urlNameString = if (param != null) {
			param.forEach {
				paramSB.append("${URLEncoder.encode(it.key, "UTF-8")}=${URLEncoder.encode(it.value, "UTF-8")}&")
			}
			"${URLEncoder.encode(url, "UTF-8")}?$paramSB"
		} else {
